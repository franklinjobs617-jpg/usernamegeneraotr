...
const SAMPLE_CUTE = ["BunnyPaws","HoneyPop","MochiSoft","KittyWink","SugarPetal","PeachGlow","DaisyBloom","FluffyJoy","CookiePop","PandaHugs","SprinklePop","LemonPaws"];

const COMBOS = [
  { a:"Bunny",   b:"Paws",   result:"BunnyPaws",   cat:"Animal + body" },
  { a:"Honey",   b:"Pop",    result:"HoneyPop",    cat:"Sweet + energy" },
...
const TIPS = [
  ...
  { emoji:"✨",title:"One sparkle word",           body:"Words like sparkle, glitter, shimmer, and glow amplify cuteness. Use one per username — not three." },
  { emoji:"💕",title:"Keep it positive",           body:"Cute usernames work best conveying happiness, softness, or playfulness. Avoid anything that reads sarcastic — it undercuts the cute effect." },
  { emoji:"🔡",title:"CamelCase or lowercase",    body:"HoneyPaws reads playfully cute. honeypaws reads softly cute. Both work — choose based on whether you want bubbly or aesthetic." },
  { emoji:"🌈",title:"Avoid numbers if possible", body:"Numbers rarely add cuteness. If your preferred name is taken, try a synonym or different pairing before appending 123." },
];
...
const FAQS = [
  ...
  { q:"What makes a username cute?",             a:"Cute usernames combine soft, round-sounding words with wholesome imagery. Animal names (bunny, kitty, panda), sweet foods (honey, mochi, cookie), and nature words (peach, daisy, rose) are the core cute vocabulary." },
  { q:"Are cute usernames good for Instagram?",  a:"Yes, especially for lifestyle, food, animal, and personal brand accounts. A cute handle signals warmth and approachability, which can increase profile clicks from new visitors." },
    ...
    <>
      ...
      <section style={{ background:"linear-gradient(160deg,#fff1f2 0%,#ffe4e6 60%,#fdf2f8 100%)" }} className="relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 py-10 md:py-14">
          ...
          <div className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full mb-4" style={{ background:"#f43f5e",color:"#fff" }}>🐾 Cute</div>
          <h1 className="text-3xl md:text-5xl font-black leading-tight tracking-tight mb-3" style={{ color:"#0d1a1a" }}>Cute Username Generator</h1>
          <p className="text-sm md:text-lg max-w-2xl mb-6" style={{ color:"#4a6060" }}>Generate adorable, sweet, and wholesome usernames for Instagram, TikTok, and any platform. Find a cute handle that makes people smile — free and instant.</p>
          <div className="mb-6">
            <p className="text-xs font-semibold mb-2" style={{ color:"#be185d" }}>Sample cute usernames:</p>
            <div className="flex flex-wrap gap-2">
              {SAMPLE_CUTE.map(n=><span key={n} className="font-mono text-xs px-2.5 py-1 rounded-lg font-semibold" style={{ background:"rgba(244,63,94,0.1)",color:"#be185d" }}>@{n}</span>)}
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <section className="section-gap">
          ...
          <p className="text-sm mb-5" style={{ color:"#4a6060" }}>Most great cute usernames follow a simple two-part formula. Here are 8 proven combinations showing exactly how to build one.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {COMBOS.map(c=>(
              <div key={c.result} className="bg-white rounded-xl px-4 py-3.5 flex items-center gap-3 flex-wrap" style={{ border:"1px solid rgba(13,122,123,0.15)",minHeight:"52px" }}>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs px-2 py-0.5 rounded" style={{ background:"#fff1f2",color:"#f43f5e" }}>{c.a}</span>
                  <span style={{ color:"#9ca3af",fontSize:"12px" }}>+</span>
                  <span className="font-mono text-xs px-2 py-0.5 rounded" style={{ background:"#fff1f2",color:"#f43f5e" }}>{c.b}</span>
                  <span style={{ color:"#9ca3af",fontSize:"12px" }}>=</span>
        ...
        <section className="section-gap"><SectionTitle>6 tips for a great cute username</SectionTitle><TipsGrid tips={TIPS} /></section>
